...
class CashMachine:
    ...
    users_data_path = data_root_path / 'users.data.csv'
    balance_suffix_path = '_balance.data.json'
    transactions_suffix_path = '_transactions.data.csv'

    transactions_column_names = ['date', 'info']
    users_column_names = ['name', 'password']

    def __init__(self):
        self.users_data = load_csv_data(self.users_data_path)
    ...
    def admin_mode(self):
        while True:
            ...
            if action == 1:
                print(self.get_available_banknotes())
            elif action == 2:
                self.load_banknotes()
            elif action == 3:
                break
    ...
    def user_mode(self, username):
        if random.randint(1, 10) == 1:
            print("Wow, you are winner !!! Your balance increased --> + 200 uah !!!")
            self._raise_users_balance(username, 200)
        while True:
            print('Select an action:')
    ...
    def raise_balance(self, name):
        ...
        self.add_info_to_csv_file(f"Increased by: {changes}. Balance: {balance}",
                                  self.data_root_path / transaction_file)

    def _raise_users_balance(self, name, count):
        balance_file = self.data_root_path / (name + self.balance_suffix_path)
        balance = load_json_data(balance_file)
        balance += count
        dump_json_data(balance, balance_file)
        transaction_file = name + self.transactions_suffix_path
        self.add_info_to_csv_file(f"BONUS. Increased by: {count}. Balance: {balance}",
                                  self.data_root_path / transaction_file)

    def withdraw(self, name):
        try:
            print(f"Possible banknotes: {list(self.get_available_banknotes().keys())}")
            changes = int(input("Enter the amount: "))
        except ValueError:
            print('Incorrect number')
            return
        ...
        dump_json_data(balance, balance_file)
        transaction_file = name + self.transactions_suffix_path
        self.add_info_to_csv_file(f"Decreased by: {changes}. Balance: {balance}",
                                  self.data_root_path / transaction_file)
    ...
    def show_history(self, name):
        ...
        print("Transactions:")
        for row in load_csv_data(self.data_root_path / transactions_file):
            print(*[row[x] for x in self.transactions_column_names], sep="  |  ")

    def get_available_banknotes(self):
        return load_json_data(self.banknotes_path)
    ...
    def add_info_to_csv_file(self, data, path):
        transactions = load_csv_data(path)

        date_time = datetime.datetime.today().replace(microsecond=0)
        new_data = {k: v for k, v in zip(self.transactions_column_names, [date_time, data])}
        transactions.append(new_data)

        dump_csv_data(transactions, path, self.transactions_column_names)
...
def load_csv_data(path: str, fieldnames: list = None):
    if not exists(path):
...
def dump_csv_data(data, path: str, fieldnames):
    with open(path, "w") as f:
        writer = csv.DictWriter(f, delimiter=',', fieldnames=fieldnames)
        writer.writeheader()
        for row in data:
